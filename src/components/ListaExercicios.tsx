...
import { Search, Filter, Dumbbell, Target, Users, Activity } from 'lucide-react';
import { movementsDatabase, getAllCategories } from '@/data/movementsDatabase';
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold text-gorila-primary mb-4">Lista Completa de Exercícios</h1>
        <p className="text-xl text-gray-600">
          Catálogo detalhado com todos os exercícios organizados por categoria
        </p>
      </div>
      ...
      <div className="mt-12 bg-gorila-primary/5 rounded-lg p-6">
        <h3 className="text-2xl font-bold text-gorila-primary mb-4 text-center">Estatísticas da Lista</h3>
      ...
      <div className="mt-8 bg-white rounded-lg p-6 shadow-sm">
        <h3 className="text-xl font-bold text-gorila-primary mb-4">Exercícios por Categoria</h3>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {categories.map(category => {
            const count = movementsDatabase.filter(e => e.categoria === category).length;
            const CategoryIcon = getCategoryIcon(category);
            return (
              <div key={category} className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg">
                <CategoryIcon className="text-gorila-primary" size={20} />
                <div>
                  <div className="font-semibold text-sm">{category}</div>
                  <div className="text-xs text-gray-600">{count} exercícios</div>
                </div>
              </div>
            );
          })}
